...
#[test]
fn test_auto_keep_alive_drop_before_sending() {
	let (pms,pmr) = mpsc::channel();
	let (pns,_) = mpsc::channel();
	let (ts,tr) = mpsc::channel();
...
fn test_check_kyokumen_nowait() {
	...
	let res = pmr.recv_timeout(Duration::from_millis(800)).expect("attempt to receive ActionKind::SetPosition timed out.");
...
fn test_go_none_limit() {
	...
	let _ = thread::spawn(move || {
		let player = MockPlayer::new(pms,pns,
										...
										ConsumedIterator::new(vec![Box::new(|player,_,limit,_,_,_,_| {
...
fn test_go_with_limit() {
	...
	let _ = thread::spawn(move || {
		let player = MockPlayer::new(pms,pns,
										...
										Box::new(|player,_,limit,_,_,_,_| {
											const EXPECTED:UsiGoTimeLimit = UsiGoTimeLimit::Limit(Some((100,200)),None);
...
fn test_go_mate() {
	...
	let _ = thread::spawn(move || {
		let player = MockPlayer::new(pms,pns,
										...
										ConsumedIterator::new(vec![]),
										ConsumedIterator::new(vec![Box::new(|player,limit,_,_,_,_| {
...
fn test_info_send_commands_without_str() {
	...
	let _ = thread::spawn(move || {
		let player = MockPlayer::new(pms,pns,
										...
										Box::new(|player,_,_,_,mut send_info_commands,_,_| {
											let _ = player.sender.send(Ok(ActionKind::Think));
...
fn test_periodically_info_send() {
	...
	if let Ok(_) = r.recv_timeout(Duration::from_millis(300)) {
		assert!(false,"It didn't time out where it was expected to time out.");
	}
