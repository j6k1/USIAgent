...
fn is_oute_moveto_gote() {
    let position_and_kinds = vec![
        ...
        (6,3,KomaKind::GKakuN),
        (4,5,KomaKind::GKakuN),
        (2,1,KomaKind::GHishaN),
        (6,1,KomaKind::GHishaN),
        (2,5,KomaKind::GHishaN),
        (6,5,KomaKind::GHishaN)
    ];
    ...
    for ((index,&m),&(x,y,kind)) in mvs.iter().enumerate().zip(position_and_kinds.iter()) {
        let (x,y) = (8-x,8-y);
