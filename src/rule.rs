...
impl State {
	...
	pub fn new(banmen:Banmen) -> State {
		...
		let mut part = PartialState {
			sente_self_board: BitBoard::from(sente_self_board),
			sente_opponent_board: BitBoard::from(sente_opponent_board),
			gote_self_board: BitBoard::from(gote_self_board),
			gote_opponent_board: BitBoard::from(gote_opponent_board),
			sente_nari_board: BitBoard::from(sente_nari_board),
			gote_nari_board: BitBoard::from(gote_nari_board),
			sente_fu_board: BitBoard::from(sente_fu_board),
			gote_fu_board: BitBoard::from(gote_fu_board),
			sente_kyou_board: BitBoard::from(sente_kyou_board),
			gote_kyou_board: BitBoard::from(gote_kyou_board),
			sente_kei_board: BitBoard::from(sente_kei_board),
			gote_kei_board: BitBoard::from(gote_kei_board),
			sente_gin_board: BitBoard::from(sente_gin_board),
			gote_gin_board: BitBoard::from(gote_gin_board),
			sente_kin_board: BitBoard::from(sente_kin_board),
			gote_kin_board: BitBoard::from(gote_kin_board),
			sente_kaku_board: BitBoard::from(sente_kaku_board),
			gote_kaku_board: BitBoard::from(gote_kaku_board),
			sente_hisha_board: BitBoard::from(sente_hisha_board),
			gote_hisha_board: BitBoard::from(gote_hisha_board),
			sente_opponent_ou_position_board: BitBoard::from(sente_opponent_ou_position_board),
			gote_opponent_ou_position_board: BitBoard::from(gote_opponent_ou_position_board),
			sente_pin_board: BitBoard::default(),
...
impl PartialState {
	...
	fn update_pin(&mut self) {
		let mut sente_pin_board = BitBoard::default();
		let mut gote_pin_board = BitBoard::default();
		...
		if let Some(to) = self.gote_opponent_ou_position_board.iter().next() {
			...
			for p in self.gote_hisha_board.iter() {
				...
				let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top(
					BitBoard::default(),BitBoard::default(), p
				) & Rule::gen_candidate_bits_by_hisha_or_kyou_to_top(
					BitBoard::default(),BitBoard::default(), 80 - to
				).reverse();
				...
				let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top(
					BitBoard::default(),BitBoard::default(), 80 - p
				).reverse() & Rule::gen_candidate_bits_by_hisha_or_kyou_to_top(
					BitBoard::default(), BitBoard::default(), to
				);
	...
	fn init_sente_checked(&mut self) {
		...
		b |= Rule::gen_candidate_bits_of_check(
			...
			BitBoard::from(FU_REV_MASK),
			self.sente_fu_board & !self.sente_nari_board
		);

		b |= Rule::gen_candidate_bits_of_check(
			self.sente_opponent_ou_position_board,
			BitBoard::from(KIN_REV_MASK),
			self.sente_fu_board & self.sente_nari_board
		);
		...
		b |= Rule::gen_candidate_bits_of_check(
			self.sente_opponent_ou_position_board,
			BitBoard::from(NARI_KAKU_REV_MASK),
		...
		b |= Rule::gen_candidate_bits_of_check(
			self.sente_opponent_ou_position_board,
			BitBoard::from(NARI_HISHA_REV_MASK),
			self.sente_hisha_board & self.sente_nari_board
		);

		b |= Rule::gen_candidate_bits_of_check(
			self.sente_opponent_ou_position_board,
			BitBoard::from(OU_REV_MASK),
...
pub const KAKU_TO_RIGHT_TOP_MASK_MAP:[u128;81] = [
	TO_RIGHT_TOP_MASK[0],
	TO_RIGHT_TOP_MASK[1],
	TO_RIGHT_TOP_MASK[2],
	TO_RIGHT_TOP_MASK[3],
...
pub const KAKU_TO_RIGHT_BOTTOM_MASK_MAP:[u128;81] = [
...
pub const V_MASK: u128 = 0b111111111;
pub const H_MASK: u128 = 0b000000001_000000001_000000001_000000001_000000001_000000001_000000001_000000001_000000001;
...
const NARI_MASK: u128 = SENTE_NARI_MASK;
const INV_NARI_MASK: u128 = GOTE_NARI_MASK;
...
/// 先手側が後手王を詰ませる可能性のある駒の位置の範囲のビットマスク
...
impl EvasionsMoveGenerator {
	...
	pub fn generate_fu<'a,B,AS: AppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
		...
		if teban == Teban::Sente {
			...
			for checked in state.part.gote_checked_board.reverse().iter() {
				...
				let mask = Rule::adjust_rev_mask(BitBoard::from(KIN_REV_MASK),checked as u32);
	...
	pub fn generate_gin<'a,B,AS: AppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
		...
		if teban == Teban::Sente {
			...
			for checked in state.part.gote_checked_board.reverse().iter() {
				let mask = Rule::adjust_rev_mask(BitBoard::from(GIN_REV_MASK),checked as u32);
	...
	pub fn generate_kaku<'a, B,AS: AppendStrategy>(teban: Teban, state: &State, move_builder: &B,mvs: &mut impl MovePicker<LegalMove>)
		...
		if teban == Teban::Sente {
			...
			for checked in state.part.gote_checked_board.reverse().iter() {
				...
				let mask = Rule::adjust_rev_mask(BitBoard::from(NARI_KAKU_REV_MASK),checked as u32);
	...
	pub fn generate_hisha<'a, B,AS: AppendStrategy>(teban: Teban, state: &State, move_builder: &B,mvs: &mut impl MovePicker<LegalMove>)
		...
		if teban == Teban::Sente {
			...
			for checked in state.part.gote_checked_board.reverse().iter() {
				...
				let mask = Rule::adjust_rev_mask(BitBoard::from(NARI_HISHA_REV_MASK),checked as u32);
...
impl ChecksMoveGenerator {
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_fu<'a,B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
												-> Result<(), LimitSizeError> where B:  Fn(u32,u32,bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
				let rev_check_mask = Rule::adjust_rev_mask(BitBoard::from(FU_REV_MASK),o as u32);
				...
				for p in (state.part.sente_fu_board & !state.part.sente_nari_board).iter() {
					...
					let candidates = Rule::gen_candidate_bits(
				...
				let rev_check_mask = Rule::adjust_rev_mask(BitBoard::from(KIN_REV_MASK),o as u32);
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_kyou<'a,B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
												  -> Result<(), LimitSizeError> where B:  Fn(u32,u32,bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_kei<'a,B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
												 -> Result<(), LimitSizeError> where B:  Fn(u32,u32,bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
				let rev_check_mask = Rule::adjust_kei_rev_mask(o as u32);
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_gin<'a,B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder:&B,mvs: &mut impl MovePicker<LegalMove>)
												 -> Result<(), LimitSizeError> where B:  Fn(u32,u32,bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
				let rev_check_mask = Rule::adjust_rev_mask(BitBoard::from(GIN_REV_MASK),o as u32);
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_kin<'a, B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder: &B,mvs: &mut impl MovePicker<LegalMove>)
												  -> Result<(), LimitSizeError> where B: Fn(u32, u32, bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_kaku<'a, B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder: &B,mvs: &mut impl MovePicker<LegalMove>)
												   -> Result<(), LimitSizeError> where B: Fn(u32, u32, bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
				for p in (state.part.sente_kaku_board & !state.part.sente_nari_board).iter() {
					...
					let board = Rule::gen_candidate_bits_by_kaku_to_right_bottom(
						state.part.gote_opponent_board,
						state.part.gote_self_board,80 - p
					) & (rev_check_mask | nari_check_mask).reverse();
					...
					) & (rev_check_mask | nari_check_mask);
	...
	/// [`LimitSizeError`]: ../error/struct.LimitSizeError.html
	#[inline]
	pub fn generate_hisha<'a, B,AS: ChecksAppendStrategy>(teban: Teban, state: &State, move_builder: &B,mvs: &mut impl MovePicker<LegalMove>)
													-> Result<(), LimitSizeError> where B: Fn(u32, u32, bool) -> LegalMove + 'a {
		if teban == Teban::Sente {
			if let Some(o) = state.part.sente_opponent_ou_position_board.iter().next() {
				for p in (state.part.sente_hisha_board & !state.part.sente_nari_board).iter() {
		...
		if teban == Teban::Sente {
			if count > 0 {
				if let Some(p) = state.part.sente_opponent_ou_position_board.iter().next() {
					...
					let candidate_bitboard = !(
						state.part.sente_self_board | state.part.sente_opponent_board
					) &	BANMEN_MASK & !(DENY_MOVE_SENTE_FU_AND_KYOU_MASK << 1) & !(mask << 1) & rev_check_mask;
...
impl GenerateStrategy for NonEvasionsAll {
	...
	fn generate_drop_gin(teban: Teban, state: &State, count: usize,
		...
		MoveGenerator::generate_drop_common(teban,state,MochigomaKind::Gin, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kin(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						 _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		MoveGenerator::generate_drop_common(teban,state,MochigomaKind::Kin, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kaku(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						  _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		MoveGenerator::generate_drop_common(teban,state,MochigomaKind::Kaku, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_hisha(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						   _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		MoveGenerator::generate_drop_common(teban,state,MochigomaKind::Hisha, count,shared_candidatebits,mvs)
	}
}
...
impl GenerateStrategy for EvasionsAll {
	...
	fn generate_drop_gin(teban: Teban, state: &State, count: usize,
						 shared_candidatebits: &mut BitBoard, _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		EvasionsMoveGenerator::generate_drop_common(teban,state,MochigomaKind::Gin, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kin(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						 _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		EvasionsMoveGenerator::generate_drop_common(teban,state,MochigomaKind::Kin, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kaku(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						  _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		EvasionsMoveGenerator::generate_drop_common(teban,state,MochigomaKind::Kaku, count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_hisha(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						   _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		EvasionsMoveGenerator::generate_drop_common(teban,state,MochigomaKind::Hisha, count,shared_candidatebits,mvs)
	}
}
pub struct Evasions;
...
pub struct Checks;
impl GenerateStrategy for Checks {
	type Environment = ();
	type AppendStrategy = ChecksForcePromotions;
	...
	fn generate_piece(teban: Teban, state: &State, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		...
		ChecksMoveGenerator::generate_fu::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_ou::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_kyou::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_kei::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_gin::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_kin::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_kaku::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
		ChecksMoveGenerator::generate_hisha::<_,Self::AppendStrategy>(teban,state,&move_builder,mvs)?;
	...
	#[inline]
	fn generate_drop(teban: Teban, state: &State, mc: &MochigomaCollections, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop(teban,state,mc,mvs)
	}
	...
	fn generate_drop_fu(teban: Teban, state: &State, count: usize,_: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>)
						-> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_fu(teban,state,count,mvs)
	}

	#[inline]
	fn generate_drop_kyou(teban: Teban, state: &State, count: usize, _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>)
						  -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_kyou(teban,state,count,mvs)
	}

	#[inline]
	fn generate_drop_kei(teban: Teban, state: &State, count: usize, _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>)
						 -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_kei(teban,state,count,mvs)
	}

	#[inline]
	fn generate_drop_gin(teban: Teban, state: &State, count: usize,
						 shared_candidatebits: &mut BitBoard, _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_gin(teban,state,count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kin(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						 _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_kin(teban,state,count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_kaku(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						  _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_kaku(teban,state,count,shared_candidatebits,mvs)
	}

	#[inline]
	fn generate_drop_hisha(teban: Teban, state: &State, count: usize, shared_candidatebits: &mut BitBoard,
						   _: &mut Self::Environment, mvs: &mut impl MovePicker<LegalMove>) -> Result<(), LimitSizeError> {
		ChecksMoveGenerator::generate_drop_hisha(teban,state,count,shared_candidatebits,mvs)
	}
}
...
impl Rule {
	...
	pub fn gen_target_attack_mask_by_kyou(
		...
		if target_mask & m != 0 {
			return (m & !flip_self_occupied_board).reverse();
		}
	...
	/// 手番側の飛車、角、香車による王手を相手番側が駒を動かす手でブロック可能かどうかを返す
	///
	/// # Arguments
	/// * `teban` - 王手をかけている手番
	/// * `state` - 盤面の状態
		...
		if teban == Teban::Sente {
			...
			match m {
				LegalMove::Put(mv) => {
					...
					Rule::can_move_for_blocking_sente(state,
								 check_line,
								 self_occupied_board,
			...
			match m {
				LegalMove::Put(mv) => {
					...
					Rule::can_move_for_blocking_gote(state,
								 check_line,
								 self_occupied_board,
								 opponent_occupied_board,
								 flip_self_occupied_board,
	...
	/// 先手側の飛車、角、香車による王手を相手番側が駒を動かす手でブロック可能かどうかを返す
	///
	/// # Arguments
	/// * `state` - 盤面の状態
	/// * `check_line` - 離れたマスからの王手のライン
	/// * `self_occupied_board` - 先手視点で見た先手側の駒の位置
	/// * `opponent_occupied_board` - 先手視点で見た後手側の駒の位置
	/// * `flip_self_occupied_board` - 後手視点で見た先手側の駒の位置
	/// * `flip_opponent_occupied_board` - 後手視点で見た後手側の駒の位置
	/// * `captured_mask` - 直前に先手が取った駒のマスク(駒の位置のビットのみが0になる)
	/// * `attacker` - 王手をかけている駒の位置
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn can_move_for_blocking_sente(state:&State,
					check_line:BitBoard,
					self_occupied_board:BitBoard,
					opponent_occupied_board:BitBoard,
					flip_self_occupied_board:BitBoard,
		...
		for p in (state.get_part().gote_kyou_board &
			...
			if Rule::gen_control_bits_by_kyou(
				...
				self_occupied_board,
				80 - p as u32
			).reverse() & check_line != 0 {
				return true;
			}
		}

		if Rule::gen_fu_reachable_block_mask(teban,state,attacker) &
			(state.get_part().gote_fu_board & captured_mask) != 0 {
			return true;
		}

		let possible_block_mask = Rule::gen_possible_block_mask(teban,state,attacker);

		for p in (state.get_part().gote_gin_board &
			!state.get_part().gote_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(80 - p as u32, KomaKind::GGin) & check_line != 0 {
				return true;
			}
		}
		...
		for p in (state.get_part().gote_kaku_board &
			state.get_part().gote_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(80 - p as u32, KomaKind::GKakuN) & check_line != 0 {
				return true;
			}
		}

		for p in (state.get_part().gote_hisha_board &
			state.get_part().gote_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(80 - p as u32, KomaKind::GHishaN) & check_line != 0 {
				return true;
			}
		}

		let possible_block_mask = Rule::gen_kei_possible_block_mask(teban,state,attacker);

		for p in (state.get_part().gote_kei_board &
			!state.get_part().gote_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(80 - p as u32, KomaKind::GKei) & check_line != 0 {
				return true;
			}
		}
	...
	/// 後手側の飛車、角、香車による王手を相手番側が駒を動かす手でブロック可能かどうかを返す
	///
	/// # Arguments
	/// * `state` - 盤面の状態
	/// * `check_line` - 離れたマスからの王手のライン
	/// * `self_occupied_board` - 後手視点で見た後手側の駒の位置
	/// * `opponent_occupied_board` - 後手視点で見た先手側の駒の位置
	/// * `flip_self_occupied_board` - 先手視点で見た後手側の駒の位置
	/// * `flip_opponent_occupied_board` - 先手視点で見た先手側の駒の位置
	/// * `captured_mask` - 直前に先手が取った駒のマスク(駒の位置のビットのみが0になる)
	/// * `attacker` - 王手をかけている駒の位置
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	fn can_move_for_blocking_gote(state:&State,
					check_line:BitBoard,
					self_occupied_board:BitBoard,
					opponent_occupied_board:BitBoard,
					flip_self_occupied_board:BitBoard,
		...
		if Rule::gen_fu_reachable_block_mask(teban,state,attacker) &
			(state.get_part().sente_fu_board & captured_mask) != 0 {
			return true;
		}

		let possible_block_mask = Rule::gen_possible_block_mask(teban,state,attacker);

		for p in (state.get_part().sente_gin_board &
			!state.get_part().sente_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(p as u32, KomaKind::SGin) & check_line != 0 {
				return true;
			}
		}
		...
		for p in (state.get_part().sente_kaku_board &
			state.get_part().sente_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(p as u32, KomaKind::SKakuN) & check_line != 0 {
				return true;
			}
		}

		for p in (state.get_part().sente_hisha_board &
			state.get_part().sente_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(p as u32, KomaKind::SHishaN) & check_line != 0 {
				return true;
			}
		}
		...
		for p in (state.get_part().sente_kei_board &
			!state.get_part().sente_nari_board &
			captured_mask & possible_block_mask).iter() {
			if Rule::gen_control_bits(p as u32, KomaKind::SKei) & check_line != 0 {
				return true;
			}
		}
	...
	pub fn gen_check_line(teban:Teban,state:&State,m:LegalMove) -> (BitBoard,u32) {
		if teban == Teban::Sente {
			...
			let op = if let Some(op) = state.get_part().sente_opponent_ou_position_board.iter().next() {
				op
			} else {
				return (BitBoard::default(),m.dst());
			};
			...
			match m {
				LegalMove::Put(mv) => {
					...
					let mut check_line = if kind == MochigomaKind::Kyou {
						Rule::gen_target_attack_mask_by_kyou(op as u32,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,to)
					} else if kind == MochigomaKind::Kaku {
						Rule::gen_target_attack_mask_by_kaku(op as u32,
															 self_occupied_board,
															 opponent_occupied_board,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,to)
					} else if kind == MochigomaKind::Hisha {
				...
				LegalMove::To(mv) => {
					...
					let pin_board = state.get_part().sente_pin_board;
					...
					let self_check_line = if (state.get_part().sente_kyou_board & !state.get_part().sente_nari_board) & from_mask != 0 {
						Rule::gen_target_attack_mask_by_kyou(op as u32,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,to)
					} else if state.get_part().sente_kaku_board & from_mask != 0 {
						Rule::gen_target_attack_mask_by_kaku(op as u32,
															 self_occupied_board,
															 opponent_occupied_board,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,to)
					} else if state.get_part().sente_hisha_board & from_mask != 0 {
						Rule::gen_target_attack_mask_by_hisha(op as u32,
															  self_occupied_board,
															  opponent_occupied_board,
															  flip_self_occupied_board,
			...
			let op = if let Some(op) = state.get_part().gote_opponent_ou_position_board.iter().next() {
				op
			} else {
				return (BitBoard::default(),m.dst());
			};
			...
			match m {
				LegalMove::Put(mv) => {
					...
					let mut check_line = if kind == MochigomaKind::Kyou {
						Rule::gen_target_attack_mask_by_kyou(op as u32,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,80 - to)
					} else if kind == MochigomaKind::Kaku {
						Rule::gen_target_attack_mask_by_kaku(op as u32,
															 self_occupied_board,
															 opponent_occupied_board,
				...
				LegalMove::To(mv) => {
					...
					let pin_board = state.get_part().gote_pin_board;
					...
					let	self_check_line = if (state.get_part().gote_kyou_board & !state.get_part().gote_nari_board) & from_mask != 0 {
						Rule::gen_target_attack_mask_by_kyou(op as u32,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,80 - to)
					} else if state.get_part().gote_kaku_board & from_mask != 0 {
						Rule::gen_target_attack_mask_by_kaku(op as u32,
															 self_occupied_board,
															 opponent_occupied_board,
															 flip_self_occupied_board,
															 flip_opponent_occupied_board,80 - to)
					} else if state.get_part().gote_hisha_board & from_mask != 0 {
						Rule::gen_target_attack_mask_by_hisha(op as u32,
															  self_occupied_board,
															  opponent_occupied_board,
															  flip_self_occupied_board,
	...
	/// pin駒の移動によってかかる角による王手のビットボードを生成
	///
	/// # Arguments
	///
	/// * `ou_position_board` - 王の位置を表すビットボード
	...
	pub fn gen_candidate_bits_of_check_after_pin_moved_by_kaku(
		...
		if let Some(p) = ou_position_board.iter().next() {
			...
			if m & pin_board != 0 && m & candidate != 0 {
				return m & candidate;
			}
	...
	/// pin駒の移動にょってかかる飛車による王手のビットボードを生成
	///
	/// # Arguments
	///
	/// * `ou_position_board` - 王の位置を表すビットボード
	...
	/// pin駒の移動に伴って消滅する先手の香車の効きのビットボードを生成
	/// # Arguments
	/// * `state` - 盤面の状態
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_sente_kyou_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.sente_checked_board & ps.sente_kyou_board & !ps.sente_nari_board).iter() {
			let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
			...
			if ps.sente_pin_board & b == 0 {
				continue;
			}

			if let Some(pin) = (ps.sente_pin_board & b).iter().next() {
				let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
					ps.gote_opponent_board,
					ps.gote_self_board,
					80 - pin as u32
	...
	/// pin駒の移動に伴って消滅する先手の角の王に対する効きのビットボードを生成
	/// # Arguments
	/// * `state` - 盤面の状態
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_sente_kaku_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.sente_checked_board & ps.sente_kaku_board).iter() {
			let b = Rule::gen_candidate_bits_by_kaku_to_right_bottom_include(
				ps.sente_self_board,
	...
	/// pin駒の移動に伴って消滅する先手の飛車の王に対する効きのビットボードを生成
	/// # Arguments
	/// * `state` - 盤面の状態
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_sente_hisha_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.sente_checked_board & ps.sente_hisha_board).iter() {
			let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
				ps.sente_self_board,
				ps.sente_opponent_board,
				p as u32
	...
	/// pin駒の移動に伴って消滅する先手の香車の後手の王に対する効きのビットボードを生成
	/// # Arguments
	/// * `state` - 盤面の状態
	///
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_gote_kyou_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.gote_checked_board.reverse() & ps.gote_kyou_board & !ps.gote_nari_board).reverse().iter() {
			let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
				ps.sente_opponent_board,
				ps.sente_self_board,
				80 - p as u32
	...
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_gote_kaku_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.gote_checked_board.reverse() & ps.gote_kaku_board).reverse().iter() {
			let b = Rule::gen_candidate_bits_by_kaku_to_right_bottom_include(
				ps.gote_self_board,
				ps.gote_opponent_board,
				p as u32
	...
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_check_removed_of_gote_hisha_by_new_pin(ps:&PartialState) -> BitBoard {
		let mut r = BitBoard::default();

		for p in (ps.gote_checked_board.reverse() & ps.gote_hisha_board).reverse().iter() {
			let b = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
				ps.gote_self_board,
				ps.gote_opponent_board,
				p as u32
	...
	/// 駒が成る手のみで王手になる手を抽出する生成された手の座標系が手番と逆側の場合のマスクを生成する
	///
	/// # Arguments
	///
	/// * `ou_position` - 王の位置
	...
	/// 飛車の駒が成る手のみで王手になる手を抽出するマスクを生成する
	///
	/// # Arguments
	///
	/// * `ou_position` - 王の位置
	...
	fn gen_candidate_bits_by_kaku_to_right_top_include(
		...
		let mut occ = self_occupied | opponent_occupied;
	...
	fn gen_candidate_bits_by_hisha_or_kyou_to_top_include(
		...
		let occ = flip_self_occupied | flip_opponent_occupied;
	...
	#[inline]
	pub fn default_moveto_builder<'a>(banmen:&'a Banmen,opponent_bitboard:BitBoard) -> impl Fn(u32,u32,bool) -> LegalMove + 'a {
		move |from,to,nari| {
			let to_mask = 1 << (to + 1);
	...
	pub fn legal_moves_with_point_and_kind_and_buffer(
		...
		let (self_bitboard,opponent_bitboard) = if kind < GFu {
			...
			(state.part.gote_self_board, state.part.sente_self_board)
		} else {
			return;
		};
	...
	pub fn win_only_move_once_with_point_and_kind_and_bitboard(
		...
		if opponent_ou_bitboard & board != 0 {
			let board = opponent_ou_bitboard;
	...
	pub fn win_only_move_sente_kaku_with_point_and_kind_and_bitboard(
	...
	) -> Option<Square> {
		{
			...
			let b = opponent_ou_bitboard & board;
	...
	pub fn apply_move_to_partial_state_none_check(state:&State,t:Teban,_:&MochigomaCollections,m:AppliedMove)
				...
				match m {
					AppliedMove::To(m) => {
						...
						let obtained = if kind < GFu {
							...
							if nari {
								ps.sente_nari_board = ps.sente_nari_board ^ to_mask;
							} else if from_mask & ps.sente_nari_board != 0 {
								ps.sente_nari_board = ps.sente_nari_board ^ (from_mask | to_mask);
							};
							(ps.sente_opponent_board & to_mask) != 0
						} else if kind < Blank {
							ps.gote_self_board = ps.gote_self_board ^ (inverse_from_mask | inverse_to_mask);
							ps.sente_opponent_board = ps.sente_opponent_board ^ (from_mask  | to_mask);

							if nari {
								ps.gote_nari_board = ps.gote_nari_board ^ to_mask;
							} else if from_mask & ps.gote_nari_board != 0 {
								ps.gote_nari_board = ps.gote_nari_board ^ (from_mask | to_mask);
							};
							(ps.gote_opponent_board & inverse_to_mask) != 0
						} else {
							false
						};
						...
						match kind {
							SFu | SFuN => {
								ps.sente_fu_board = ps.sente_fu_board ^ (from_mask | to_mask);
							},
							SKyou | SKyouN => {
								ps.sente_kyou_board = ps.sente_kyou_board ^ (from_mask | to_mask);
							},
							SKei | SKeiN => {
								ps.sente_kei_board = ps.sente_kei_board ^ (from_mask | to_mask);
							},
							SGin | SGinN => {
								ps.sente_gin_board = ps.sente_gin_board ^ (from_mask | to_mask);
							},
							SKin => {
								ps.sente_kin_board = ps.sente_kin_board ^ (from_mask | to_mask);
							},
							SKaku | SKakuN => {
								ps.sente_kaku_board = ps.sente_kaku_board ^ (from_mask | to_mask);
							},
							SHisha | SHishaN => {
								ps.sente_hisha_board = ps.sente_hisha_board ^ (from_mask | to_mask);
							},
							SOu => {
								ps.gote_opponent_ou_position_board = ps.gote_opponent_ou_position_board ^ (inverse_from_mask | inverse_to_mask);
							},
							GFu | GFuN => {
								ps.gote_fu_board = ps.gote_fu_board ^ (from_mask | to_mask);
							},
							GKyou | GKyouN => {
								ps.gote_kyou_board = ps.gote_kyou_board ^ (from_mask | to_mask);
							},
							GKei | GKeiN => {
								ps.gote_kei_board = ps.gote_kei_board ^ (from_mask | to_mask);
							},
							GGin | GGinN => {
								ps.gote_gin_board = ps.gote_gin_board ^ (from_mask | to_mask);
							},
							GKin => {
								ps.gote_kin_board = ps.gote_kin_board ^ (from_mask | to_mask);
							},
							GKaku | GKakuN => {
								ps.gote_kaku_board = ps.gote_kaku_board ^ (from_mask | to_mask);
							},
							GHisha | GHishaN => {
								ps.gote_hisha_board = ps.gote_hisha_board ^ (from_mask | to_mask);
							},
							GOu => {
								ps.sente_opponent_ou_position_board = ps.sente_opponent_ou_position_board ^ (from_mask | to_mask);
							},
							Blank => (),
						}
						...
						if obtained {
							...
							let inverse_obtained_mask = !inverse_to_mask;
							...
							if kind < GFu {
								...
								ps.gote_self_board = ps.gote_self_board & inverse_obtained_mask;
								ps.sente_opponent_board  = ps.sente_opponent_board & obtained_mask;
								ps.gote_nari_board = ps.gote_nari_board & obtained_mask;
								ps.gote_checked_board &= inverse_obtained_mask;
							}

							match kind {
								SFu | SFuN => {
									ps.sente_fu_board = ps.sente_fu_board & obtained_mask;
								},
								SKyou | SKyouN => {
									ps.sente_kyou_board = ps.sente_kyou_board & obtained_mask;
								},
								SKei | SKeiN => {
									ps.sente_kei_board = ps.sente_kei_board & obtained_mask;
								},
								SGin | SGinN => {
									ps.sente_gin_board = ps.sente_gin_board & obtained_mask;
								},
								SKin => {
									ps.sente_kin_board = ps.sente_kin_board & obtained_mask;
								},
								SKaku | SKakuN => {
									ps.sente_kaku_board = ps.sente_kaku_board & obtained_mask;
								},
								SHisha | SHishaN => {
									ps.sente_hisha_board = ps.sente_hisha_board & obtained_mask;
								}
								SOu => {
									ps.gote_opponent_ou_position_board = ps.gote_opponent_ou_position_board & inverse_obtained_mask;
								},
								GFu | GFuN => {
									ps.gote_fu_board = ps.gote_fu_board & obtained_mask;
								},
								GKyou | GKyouN => {
									ps.gote_kyou_board = ps.gote_kyou_board & obtained_mask;
								},
								GKei | GKeiN => {
									ps.gote_kei_board = ps.gote_kei_board & obtained_mask;
								},
								GGin | GGinN => {
									ps.gote_gin_board = ps.gote_gin_board & obtained_mask;
								},
								GKin => {
									ps.gote_kin_board = ps.gote_kin_board & obtained_mask;
								},
								GKaku | GKakuN => {
									ps.gote_kaku_board = ps.gote_kaku_board & obtained_mask;
								},
								GHisha | GHishaN => {
									ps.gote_hisha_board = ps.gote_hisha_board & obtained_mask;
								},
								GOu => {
									ps.sente_opponent_ou_position_board = ps.sente_opponent_ou_position_board & obtained_mask;
								},
								Blank => ()
							}
						}
						...
						ps.sente_checked_board = ps.sente_checked_board & !from_mask;
						ps.gote_checked_board = ps.gote_checked_board & !inverse_from_mask;
						...
						match kind {
							...
							SOu => {
								b |= Rule::gen_candidate_bits_of_check(
									ps.sente_opponent_ou_position_board.reverse(),
									BitBoard::from(OU_REV_MASK),
									BitBoard::from(inverse_to_mask)
								).reverse();
							...
							GOu => {
								b |= Rule::gen_candidate_bits_of_check(
									ps.gote_opponent_ou_position_board.reverse(),
									BitBoard::from(OU_REV_MASK),
									BitBoard::from(to_mask)
								).reverse();
						...
						match t {
							Teban::Sente => {
								...
								match m.kind() {
									...
									MochigomaKind::Kyou => {
										ps.sente_kyou_board = ps.sente_kyou_board ^ to_mask;
									},
									MochigomaKind::Kei => {
										ps.sente_kei_board = ps.sente_kei_board ^ to_mask;
									},
									MochigomaKind::Gin => {
										ps.sente_gin_board = ps.sente_gin_board ^ to_mask;
									},
									MochigomaKind::Kin => {
										ps.sente_kin_board = ps.sente_kin_board ^ to_mask;
									},
									MochigomaKind::Kaku => {
										ps.sente_kaku_board = ps.sente_kaku_board ^ to_mask;
									},
									MochigomaKind::Hisha => {
										ps.sente_hisha_board = ps.sente_hisha_board ^ to_mask;
									}
								}
							},
							Teban::Gote => {
								...
								match m.kind() {
									MochigomaKind::Fu => {
										ps.gote_fu_board = ps.gote_fu_board ^ to_mask;
									},
									MochigomaKind::Kyou => {
										ps.gote_kyou_board = ps.gote_kyou_board ^ to_mask;
									},
									MochigomaKind::Kei => {
										ps.gote_kei_board = ps.gote_kei_board ^ to_mask;
									},
									MochigomaKind::Gin => {
										ps.gote_gin_board = ps.gote_gin_board ^ to_mask;
									},
									MochigomaKind::Kin => {
										ps.gote_kin_board = ps.gote_kin_board ^ to_mask;
									},
									MochigomaKind::Kaku => {
										ps.gote_kaku_board = ps.gote_kaku_board ^ to_mask;
									},
									MochigomaKind::Hisha => {
										ps.gote_hisha_board = ps.gote_hisha_board ^ to_mask;
									}
								}
							}
						}
		...
		ps.sente_checked_board &= !(Rule::gen_check_removed_of_sente_kyou_by_new_pin(&ps) |
			Rule::gen_check_removed_of_sente_kaku_by_new_pin(&ps) |
			Rule::gen_check_removed_of_sente_hisha_by_new_pin(&ps));

		ps.gote_checked_board &= !(Rule::gen_check_removed_of_gote_kyou_by_new_pin(&ps) |
	...
	pub fn is_nyugyoku_win(state:&State,t:Teban,mc:&MochigomaCollections,limit:&Option<Instant>) -> bool {
		...
		match t {
			Teban::Sente => {
				if state.part.gote_opponent_ou_position_board & NYUGYOKU_MASK << 1 == 0 {
					return false;
				}
			},
			Teban::Gote => {
				if state.part.sente_opponent_ou_position_board & NYUGYOKU_MASK << 1 == 0 {
					return false;
				}
			},
		}
		...
		match t {
			Teban::Sente => {
				...
				let sente_hisha_board = state.part.sente_hisha_board;
				let sente_kaku_board = state.part.sente_kaku_board;

				let sente_occupied_board = sente_occupied_board &
						!(sente_hisha_board | sente_kaku_board | ou_bitboard) & (SENTE_NARI_MASK << 1);

				let sente_oogoma_board = (sente_hisha_board | sente_kaku_board) & (SENTE_NARI_MASK << 1);
			...
			Teban::Gote => {
				let ou_bitboard = state.part.sente_opponent_ou_position_board;
				...
				let gote_hisha_board = state.part.gote_hisha_board;
				let gote_kaku_board = state.part.gote_kaku_board;

				let gote_occupied_board = gote_occupied_board &
						!(gote_hisha_board | gote_kaku_board | ou_bitboard) & (GOTE_NARI_MASK << 1);

				let gote_oogoma_board = (gote_hisha_board | gote_kaku_board) & (GOTE_NARI_MASK << 1);
	...
	pub fn is_put_fu_and_mate_sente(state:&State,p:u32) -> bool {
		...
		let filter = if p < 10 {
			BitBoard::from(POSSIBLE_EVASIONS_BY_FU_CAPTURE_MASK >> (10 - p))
		} else {
			BitBoard::from(POSSIBLE_EVASIONS_BY_FU_CAPTURE_MASK << (p - 10))
		};
		...
		let filter = if p < 20 {
			BitBoard::from(POSSIBLE_OU_CAPTURES_MASK_OF_SENTE >> (20 - p))
		} else {
			BitBoard::from(POSSIBLE_OU_CAPTURES_MASK_OF_SENTE << (p - 20))
		};
	...
	pub fn is_put_fu_and_mate_gote(state:&State,p:u32) -> bool {
		...
		let filter = if p < 9 {
			BitBoard::from(POSSIBLE_EVASIONS_BY_FU_CAPTURE_MASK >> (9 - p))
		} else {
			BitBoard::from(POSSIBLE_EVASIONS_BY_FU_CAPTURE_MASK << (p - 9))
		};
		...
		let filter = if p < 19 {
			BitBoard::from(POSSIBLE_OU_CAPTURES_MASK_OF_GOTE >> (19 - p))
		} else {
			BitBoard::from(POSSIBLE_OU_CAPTURES_MASK_OF_GOTE << (p - 19))
		};
	...
	pub fn in_check_with_partial_state_repeat_move_kinds(t:Teban, ps:&PartialState) -> bool {
		match t {
			Teban::Gote => {
				...
				let bitboard = ps.sente_kyou_board & !ps.sente_nari_board;
			...
			Teban::Sente => {
				...
				let bitboard = ps.gote_kyou_board & !ps.gote_nari_board;
	...
	pub fn has_control_gote_kyou(state:&State,to:Square) -> bool {
		...
		for p in (state.part.gote_kyou_board & !state.part.gote_nari_board).reverse().iter() {
			...
			let from_idx = 80 - fp;
			let cand = Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(
				state.part.sente_opponent_board, state.part.sente_self_board, from_idx).reverse();
			if (cand & to_bb) != 0 { return true; }
		}
		false
	}
	...
	pub fn has_control_gote_kaku(state:&State,to:Square) -> bool {
		...
		for p in (state.part.gote_kaku_board & !state.part.gote_nari_board).reverse().iter() {
			...
			let from_idx = 80 - fp;
			let cand =
				Rule::gen_candidate_bits_by_kaku_to_right_bottom_include(state.part.gote_self_board, state.part.gote_opponent_board, 80 - from_idx)
				| Rule::gen_candidate_bits_by_kaku_to_right_top_include(state.part.gote_self_board, state.part.gote_opponent_board, 80 - from_idx)
				| Rule::gen_candidate_bits_by_kaku_to_right_bottom_include(state.part.sente_opponent_board, state.part.sente_self_board, from_idx).reverse()
				| Rule::gen_candidate_bits_by_kaku_to_right_top_include(state.part.sente_opponent_board, state.part.sente_self_board, from_idx).reverse();
			if (cand & to_bb) != 0 { return true; }
		}
		false
	}
	...
	pub fn has_control_gote_hisha(state:&State,to:Square) -> bool {
		...
		for p in (state.part.gote_hisha_board & !state.part.gote_nari_board).reverse().iter() {
			...
			let cand =
				...
				Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(state.part.gote_self_board, state.part.gote_opponent_board, 80 - from_idx)
				| Rule::gen_candidate_bits_by_hisha_or_kyou_to_top_include(state.part.sente_opponent_board, state.part.sente_self_board, from_idx).reverse()
				// horizontal in both directions
				| Rule::gen_candidate_bits_by_hisha_to_right_include(state.part.gote_self_board, state.part.gote_opponent_board, 80 - from_idx)
				| Rule::gen_candidate_bits_by_hisha_to_right_include(state.part.sente_opponent_board, state.part.sente_self_board, from_idx).reverse();
			if (cand & to_bb) != 0 { return true; }
		}
		false
	}
	...
	pub fn has_control_bits_sente_ou(state:&State,to:Square) -> BitBoard {
		let to_bb = BitBoard::from(1 << (to + 1));
		let mut res = BitBoard::default();
		let board = state.part.gote_opponent_ou_position_board;
		for p in board.reverse().iter() {
			let p = p as u32;
			let cand = Rule::gen_candidate_bits(Teban::Sente, BitBoard::default(), p, SKin);
			if (cand & to_bb) != 0 { res |= BitBoard::from(1 << (p + 1)); }
	...
	/// pin駒の移動によってかかる王手を抽出するためのビットマスクの生成
	///
	/// # Arguments
	///
	...
	pub fn gen_unpinning_reverse_check_mask_and_attacker(
		...
		if pin_board & (1 << (from + 1)) == 0 {
			...
			if self_kyou_board != 0 || self_hisha_board != 0 {
				...
				if m & pin_board != 0 {
					return Some((!m,im.reverse()
									  .iter()
									  .next()
									  .map(|p| 80 - p).expect("Could not retrieve item from logic error iterator.")));
				}
			}
			...
			if self_kaku_board != 0 {
				...
				if m & pin_board != 0 {
					return Some((!m,im.iter().next().expect("Could not retrieve item from logic error iterator.")));
				}
	...
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	pub fn gen_kyou_reverse_check_mask(
		ou_position_board:BitBoard,
		self_occupied_board:BitBoard,
		opponent_occupied_board:BitBoard
	) -> BitBoard {
	...
	/// 角の移動によって王手がかかる位置のビットマスクの生成
	///
	/// # Arguments
	///
	/// * `ou_position_board` - 王手をかける手番側から見た王の位置を表すビットボード
	/// * `from` - 王手をかける手番側視点での動かす駒の移動元。駒を打つ手の場合は-1を渡す。
	/// * `self_occupied_board` - 王手をかけようとする手番側から見た王手をかけようとする手番側のビットボード
	/// * `opponent_occupied_board` - 王手をかけようとする手番側から見た王手をかけられる側の手番のビットボード
	/// * `flip_self_occupied_board` - 王手をかけられようとする手番側から見た王手をかけられようとする手番側のビットボード
	/// * `flip_opponent_occupied_board` - 王手をかけられようとする手番側か見た王手をかけようとするる側の手番のビットボード
	/// 渡した引数の状態が不正な場合の動作は未定義
	#[inline]
	...
	/// 飛車、角、香車の王手ラインを遮断する可能性のある駒をフィルタするマスクを生成する。マスクは常に先手視点で生成される。
	///
	/// # Arguments
	///
	/// * `taban` - 王手をかけている方の手番
	...
	pub fn gen_possible_block_mask(teban:Teban,state:&State,from:u32) -> BitBoard {
		...
		if sx == tx && tx <= 7 {
			...
			(BitBoard::from(H_MASK << (ty + 1) >> 1) | BitBoard::from(H_MASK << (ty + 1 + 1))) & !1
		} else if sy == ty {
			(BitBoard::from(H_MASK << (ty + 1) >> 1)) & !1
		} else if sy < ty {
	...
	/// 飛車、角、香車の王手ラインを確実に遮断できる位置にいる歩の位置を抽出するビットボードを生成する。常に先手視点のビットボードを返す。
	///
	/// # Arguments
	///
	/// * `taban` - 王手をかけている方の手番
	...
	pub fn gen_fu_reachable_block_mask(teban:Teban,state:&State,from:u32) -> BitBoard {
		...
		if teban == Teban::Sente {
			...
			if sx == tx {
				...
				let b = BitBoard::from(1 << (t - 7 + 1));
				(b - (1 << (s as u128 + 9 + 1)) ^ b) & (BitBoard::from(KAKU_TO_RIGHT_TOP_MASK_MAP[s as usize + 9]) << 1)
			}
		}
	}
